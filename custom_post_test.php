...
// Implement display_movie_review_meta_box Function
...
// Implementation of the add_movie_review_fields Function
...
// Select Custom Posts
function custPosts() {
	$args = array( 'post_type' => 'movie_reviews', 'posts_per_page' => 10 );
	$loop = new WP_Query( $args );
	while ( $loop->have_posts() ) : $loop->the_post();
		the_title();
		echo '<div class="entry-content">';
		echo '<strong>Director: </strong>';
		echo esc_html( get_post_meta( get_the_ID(), 'movie_director', true ) );
		the_content();
		echo '</div>';
	endwhile;
}
